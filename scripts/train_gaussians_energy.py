...
from models.gaussian_energy import GaussianSplat2D
from models.render_energy import render_gaussians_efficient, compute_ssim
from utils.image_utils_energy import (
    load_image,
    save_image,
    get_image_info,
    compute_image_energy,
    sample_points_from_energy,
)
...
def train_gaussians(
    ...
    init_mode: str = "energy",  # "energy" or "uniform"
    ...
    Args:
        ...
        init_mode: "energy" to bias splats toward detailed regions, "uniform" for random
        energy_floor: uniform sampling floor for low-detail regions
        energy_power: sharpness of energy weighting (>1 concentrates more on details)
        energy_smooth: local window size for stable energy/variation estimation
    """
    ...
    if init_mode == "energy":
        print(
            f"Using energy-based initialization "
            f"(floor={energy_floor}, power={energy_power}, smooth={energy_smooth})"
        )
        energy = compute_image_energy(
            target_image,
            smooth_kernel_size=energy_smooth,
            energy_power=energy_power,
            energy_floor=energy_floor,
        )
        sampled_means = sample_points_from_energy(energy, num_gaussians)

        sample_x = torch.clamp(sampled_means[:, 0].long(), 0, img_width - 1)
        sample_y = torch.clamp(sampled_means[:, 1].long(), 0, img_height - 1)
        sampled_colors = target_image[sample_y, sample_x]
        sampled_energy = energy[sample_y, sample_x]
        sampled_energy = sampled_energy / (sampled_energy.max() + 1e-8)

        with torch.no_grad():
            gaussians.means.data = sampled_means
            clipped_colors = torch.clamp(sampled_colors, 1e-4, 1.0 - 1e-4)
            gaussians.colors.data = torch.logit(clipped_colors)

            base_scale = max(img_height, img_width) / 28.0
            scale_multiplier = 1.6 - 1.1 * sampled_energy.unsqueeze(1)
            init_scales = torch.clamp(base_scale * scale_multiplier, min=1.0)
            gaussians.log_scales.data = torch.log(init_scales).repeat(1, 2)

        energy_vis = energy / (energy.max() + 1e-8)
        save_image(energy_vis.unsqueeze(-1).repeat(1, 1, 3), str(output_dir / "energy_map.png"))
        print(f"Saved energy map preview to {output_dir / 'energy_map.png'}")
    elif init_mode != "uniform":
        raise ValueError("init_mode must be either 'energy' or 'uniform'")
    ...
    if resume and checkpoint_path.exists():
        if init_mode == "energy":
            print("Checkpoint found; saved parameters override the energy-based initialization.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--init-mode", type=str, choices=["energy", "uniform"], default="energy",
                        help="Initialize splats from image energy or uniformly at random")
    parser.add_argument("--energy-floor", type=float, default=0.02,
                        help="Uniform sampling floor for low-detail regions when using energy init")
    parser.add_argument("--energy-power", type=float, default=1.0,
                        help="Energy weighting sharpness; >1 concentrates splats more on detailed regions")
    parser.add_argument("--energy-smooth", type=int, default=9,
                        help="Local window size used to smooth the energy map")
    ...
    train_gaussians(
        ...
        energy_smooth=args.energy_smooth,
    )
